...
for (let i = 1; i <= 3000000; i++) {
    const nodeType = Math.random() < 0.5 ? "rule" : "data";
    graphData.nodes.push({
        id: i.toString(),
        type: nodeType,
        label: `${nodeType.charAt(0).toUpperCase() + nodeType.slice(1)} ${i}`,
        description: `This is ${nodeType} ${i}. It contains some sample information about the node.`
    });
}

// Generate random links
for (let i = 1; i <= 10000000; i++) {
    const numLinks = Math.floor(Math.random() * 5) + 1; // 1 to 5 links per node
    for (let j = 0; j < numLinks; j++) {
        const targetId = Math.floor(Math.random() * 200) + 1;
        if (targetId !== i) {
            const linkType = Math.random() < 0.33 ? "output" : (Math.random() < 0.66 ? "input" : "contains");
...
function updateGraph() {
    ...
    const subgraph = getSubgraph(startNodeId, forwardDepth, backwardDepth, edgeLimit);
